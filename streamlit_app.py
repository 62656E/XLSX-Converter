...
def convert_excel_to_csv(excel_file: openpyxl.Workbook) -> list[tuple[str, bytes]]:
    ...
    for sheet in enumerate(
        ...
        csv_file_name = (
            f"{excel_file.name.replace('.xlsx', '').replace('.xls', '')}_{sheet}.csv"
        )
        csv_content = df.to_csv(index=False).encode(
            "utf-8"
        )  # Convert the DataFrame to CSV and encode it to bytes
        csv_files.append(
            (csv_file_name, csv_content)
        )  # Append the CSV file name and content to the list as a tuple
    return csv_files
...
if uploaded_file:
    with st.spinner(
        "Converting the file..."
    ):  # Display a spinner while the file is being converted
        ...
        st.markdown(
            "### Download CSV Files"
        )  # Display a header for the download section
